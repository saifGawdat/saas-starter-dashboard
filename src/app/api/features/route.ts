...
export async function GET() {
  try {
    ...
    if (!hasPermission(session, PERMISSIONS.FEATURES_VIEW)) {
...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!hasPermission(session, PERMISSIONS.FEATURES_MANAGE)) {
      return NextResponse.json({ error: "Forbidden" }, { status: 403 });
    }
...
export async function PUT(req: NextRequest) {
  try {
    ...
    const hasPerm = hasPermission(session, PERMISSIONS.FEATURES_MANAGE);
